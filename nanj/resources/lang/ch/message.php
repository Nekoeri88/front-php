<?php
/**
 * 多言語化対応用に文字列は全てここに記述し、それぞれのviewファイルやcssには文字列を残さない事。
 */
return [
    'coinName' => 'NANJCOIN',
    'description' => '以NANJ网民发起的为了NANJ网民的数字货币NANJ',
    'top' => 'TOP',
    'about' => 'ABOUT',
    'roadMap' => 'ROADMAP',
    'whitePaper' => 'WHITEPAPER',
    'wallet' => 'WALLET',
    'exchanges' => 'EXCHANGES',
    'community' => 'COMMUNITY',
    'whatIsNanJ' => 'NANJ是什么？',
    'whatIsNanJDescription' =>
        'NANJCOIN是一个由日本知名网站2ch.sc上的通称为「なんJ」的网络团体发起的以太币。由其中一部分富有专业精神和志向的网民独家开发、运营。『NANJCOIN』作为一款世上罕见的专门针对体育竞技活动发行的以太币，主要受众群是喜爱各种体育竞技（尤其是棒球）的なんJ网民。NANJCOIN也在积极推动其产品的多样性，期待在未来能够应用于更多种类的体育项目，如门票及相关产品的购买支付，以及体育团队、选手个人捐助等方面。目前已于2018年2月实行了免费赠送活动，即将在国际虚拟货币交易所上市并开始预售。<br>※NANJCOIN是由粉丝团体发行，与2ch.sc及5ch等网站的运营团队无关。',
    'whatIsNanJDescription2' =>
        'NANJCOIN是日本首款基于ERC223标准开发的以太币。以“更好地享受体育竞技”的主题为基础，将体育竞技相关的支付方式作为重点来扩大合作关系。相比普通的购买模式，NANJCOIN的用户享有更多的优惠折扣，以及额外礼品赠送。此外，我们还将建立面向资金不足的选手或体育团队的便利捐赠系统。例如日本国内观众较少的体育项目或地区的小型俱乐部。（Stage3之后）我们计划利用NANJCOIN作为团体里的vote功能的投票权利。期待以这种方式，实现一个使用vote功能在现实中运营体育团队等的未来世界。',
    'tokenDetail' => '关于代币',
    'tokenAssign' => '代币配额',
    'tokenName' => '代币名： NANJCOIN',
    'tokenSymbol' => '代币象征： NANJ',
    'issueNumber' => '发行数量： 300亿',
    'contractAddress' => '合同地址 ：<br><span class="address">0xFFE02ee4C69eDf1b340fCaD64fbd6b37a7b9e265</span>',
    'numberOfDigits' => '小数点位数 ： 8',
    'comingSoon' => '制作中，请稍等。',
    'preSeason' => '预售',
    'operation' => '运营团队',
    'developmentFee' => '研发费用',
    'advertisementFee' => '市场营销费用',
    'nanjMean' => '<strong>N</strong>EXT-GENERATION <strong>A</strong>THLETE <strong>N</strong>ETWORK <strong>J</strong>OINT',
    'nanjMean2' => '<strong>E</strong>THEREUM <strong>E</strong>RC223 <strong>T</strong>OKEN <strong>F</strong>ULLY <strong>C</strong>OMPATIBLE <strong>W</strong>ITH <strong>E</strong>RC20 <strong>T</strong>OKEN <strong>S</strong>TANDARD',
    'assignDescription' =>
        '目前市面上虚拟货币的运营团队，所持有的代币分配率基本是不透明的，或是持有数超过一半。所以我们将NANJCOIN运营团队的所持代币配额定为<strong>全体的10%。</strong>这种分配的优点是确保了运营的透明性，并且即使运营团队把所持代币全部抛售，也不会因为超过发行数的一半而使NANJCOIN的市场价格崩溃。当然我们保证这种扰乱市场价格的行为绝不会发生。这种分配模式是我们的双重价格保护措施。我们将在预售时发售相当于总发行数60%的180亿NANJCOIN。预售结束后，未售出的代币将被销毁或禁止交易。',
    'roadMapDescription' =>
        '想看看粉丝与体育竞技的互动新境界吗？NANJCOIN计划在专业开发与营销团体的监督指导之下，坚实地踏出实现可能的每一步，造就更高的自身价值。',
    'roadmapPlace' => '/img/roadmap_img_03.png',
    'roadmapPlaceSp' => '/img/roadmap_img_03_sp.png',
    'listingApprovalPlace' => '/img/listingapproval_img_02.png',
    'listingApprovalPlaceSp' => '/img/listingapproval_img_02_sp.png',
    'listingApprovalUntil' => 'Left',
    'cryptocurrencySymbol' => 'CNY',
    'memberDescription' => '2018年3月14日、NANJ正式成立为上市公司。在以创始人森裕司为中心的、法务、开发、货币工程师等所组成的核心布局下开始运营。',
    'memberProfile01' => '
      <dd class="dd03"><span>法人代表CEO</span><span>森 裕司</span></dd>
      <dd class="dd04">・右投/右打</dd>
    ',
    'memberProfile02' => '
      <dd class="dd03"><span>法人代表CEO＆CFO</span><span>石 靖士</span></dd>
      <dd class="dd04">・右投/右打</dd>
    ',
    'memberProfile03' => '
      <dd class="dd03"><span>法人代表CEO＆CTO</span><span>小野良太</span></dd>
      <dd class="dd04">・右投/左打</dd>
    ',
    'memberProfile04' => '
      <dd class="dd03"><span>法人代表CEO＆CMO</span><span>弘田大介</span></dd>
      <dd class="dd04">・右投/右打</dd>
    ',
    'memberProfile05' => '
      <dd class="dd03"><span>董事CCO</span><span>向井利邦</span></dd>
      <dd class="dd04">・右投/右打</dd>
    ',
    'memberProfile06' => '
      <dd class="dd03"><span>董事CIO</span><span>下川和人</span></dd>
      <dd class="dd04">・右投/右打</dd>
    ',
    'memberProfile07' => '
      <dd class="dd03"><span>董事CBO</span><span>谷口征嗣</span></dd>
      <dd class="dd04">・右投/右打</dd>
    ',
    

];
